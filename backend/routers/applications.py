...
def create_application(
    ...
    if data.application_status:
        app_data["application_status"] = normalize_status(data.application_status)
...
def update_application(
    ...
    if "application_status" in update_data:
        ...
        update_data["application_status"] = normalize_status(update_data["application_status"])
        log = models.ActivityLog(
            application_id=app.id,
            changed_by_id=current_user.id,
...
def update_status(
    ...
    if not app:
        raise HTTPException(status_code=404, detail="Application not found")
    old_status = app.application_status
    app.application_status = normalize_status(data.application_status)
    log = models.ActivityLog(
        application_id=app.id,
        changed_by_id=current_user.id,
        field_name="application_status",
...
def get_logs(app_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    return (
